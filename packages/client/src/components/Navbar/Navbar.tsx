...
import { Button, ButtonGroup, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, useDisclosure } from '@chakra-ui/react'
import { GoBeaker, GoCloudDownload, GoGear, GoHome, GoListUnordered, GoPackage, GoPerson, GoPlus, GoSignIn, GoSignOut } from 'react-icons/go'
    ...
    <>
      <ButtonGroup className='Navbar' display='flex' justifyContent='space-between'>
        ...
        {isAuth ?
          <ButtonGroup variant='solid'>
            <Button leftIcon={<GoBeaker />} as={Link} to={'/app'}>Workspace</Button>
            {isLoading ?
              <Button as={Link} to={'/pages'} leftIcon={<GoListUnordered />} colorScheme='messenger' transition='all 0.3s ease-in'>My Pages</Button>
              :
              <Button as={Link} to={'/pages'} leftIcon={<GoListUnordered />} transition='all 0.3s ease-out'>My Pages</Button>
            }
          </ButtonGroup>
          :
          null
        ...
        <ButtonGroup spacing='5'>
          {userInfo.role === ADMIN ? <Button leftIcon={<GoGear />} as={Link} to={'/create_element'} colorScheme='green'>Create new element</Button> : null}

          {isAuth ?
            <>
            <ButtonGroup variant='solid'>
              <Button as='div' leftIcon={<GoPerson />} variant='outline' colorScheme='black'>{userInfo.email}</Button>
              <Button isLoading={isLoading} onClick={saveToUserPages} isDisabled={currentElements.length === 0} rightIcon={<GoPackage />}>Save to My Pages</Button>
              <Button onClick={onOpen} isDisabled={currentElements.length === 0} rightIcon={<GoCloudDownload />}>Download</Button>
      ...
      <Modal size='sm' motionPreset='scale' isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button isLoading={isDownload} onClick={downloadPage} isDisabled={pageTitle.length < 1} rightIcon={<GoCloudDownload />}>Download</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
